...
/* WRITE NUMBER */
/**
 * write_digitber - Prints a string
	...
	int flags, int width, int _preci, int size)
{
	...
	if (has_negative)
		extra_ch = '-';
	else if (flags & F_ADDITION)
		extra_ch = '+';
	else if (flags & F_WIDTH)
		extra_ch = ' ';

	return (write_digit(ind, b, flags, width, _preci,
		length, padd, extra_ch));
}

/**
 * write_digit - Prints a number using a bufffer
 * @ind: Index of buffer
 * @b: Buffer
 * @flags: Flags
 * @width: width
 ...
 * Return: Written chars count
 */
int write_digit(int ind, char b[],
	...
	int length, char padd, char extra_c)
{
	...
	if (width > length)
	{
		for (i = 1; i < width - length + 1; i++)
			b[i] = padd;
		b[i] = '\0';
		if (flags & F_SUBSTRACT && padd == ' ')
		{
			if (extra_c)
				b[--ind] = extra_c;
			return (write(1, &b[ind], length) + write(1, &b[1], i - 1));
		}
		else if (!(flags & F_SUBSTRACT) && padd == ' ')
		{
			if (extra_c)
				b[--ind] = extra_c;
			return (write(1, &b[1], i - 1) + write(1, &b[ind], length));
		}
		else if (!(flags & F_SUBSTRACT) && padd == '0')
		{
			if (extra_c)
